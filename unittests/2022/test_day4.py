...
def random_containing_set(draw):
    ...
    return (anchor, anchor+inner_size, anchor - offset, anchor - offset + outer_size)
...
def random_non_overlapping(draw):
    ...
    return (anchor - size_1, anchor, anchor + disjoint_size, anchor + disjoint_size + size_2)
...
def random_overlapping_set(draw):
    ...
    overlap_size = draw(st.integers(min_value=0, max_value=min(size_1, size_2)))

    anchor = draw(random_integer)

    return (anchor - size_1, anchor, anchor - overlap_size, anchor - overlap_size + size_2)
...
class TestDay4:
    ...
    def test_set_contains_other_set(self, random_containing_set):
        ...
        # Assert
        assert actual_contains == True

    @given(random_non_overlapping())
    def test_set_contains_other_set_non_overlapping(self, random_non_overlapping):
        # Prepare
